...
function ProgramIcon({ icon }: { icon: string }) {
  return (
    <div className="w-8 h-8 flex-shrink-0 rounded-lg bg-brand-50 dark:bg-brand-900/30 flex items-center justify-center">
      {icon === 'book' && (
...
export default function ProgramsPage() {
  return (
    <div className="min-h-screen bg-white dark:bg-slate-900">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          {displayDays.map(day => {
            ...
            return (
              <div key={day} className="border border-brand-100 dark:border-brand-900/30 rounded-lg">
                <div className="px-4 py-3 border-b border-brand-100 dark:border-brand-900/30 flex justify-between items-center">
                    ...
                    <div className="space-y-4">
                      {programs.map((program, idx) => (
                        <div key={idx} className="flex items-start space-x-3">
                          ...
                          <div>
                            <h4 className="text-sm font-medium text-brand-900 dark:text-brand-100">
                              {program.title}
                            </h4>
      ...
      {/* Program Details */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h2 className="text-2xl font-semibold text-brand-900 dark:text-white mb-6">
          Program Details
        </h2>
        <div className="grid gap-6 md:grid-cols-2">
          {weeklyPrograms.map((program, index) => (
            <div 
              key={index}
              className="border border-brand-100 dark:border-brand-900/30 rounded-lg p-5"
            >
              <div className="flex items-start space-x-4">
                <ProgramIcon icon={program.icon} />
                <div>
                  <h3 className="text-lg font-medium text-brand-900 dark:text-white mb-1">
                    {program.title}
                  </h3>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="border border-brand-100 dark:border-brand-900/30 rounded-lg p-6">
          <div className="flex items-center justify-center space-x-2 text-sm text-brand-700 dark:text-brand-300">
